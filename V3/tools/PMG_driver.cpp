...
int main(int argc, char** argv)
{
    ...
    std::vector<float> verse_rhythm{1.5, 1, 0.5, 1, 
                                    1, 0.5, 1, 0.5, 1, 
                                    1.5, 1, 0.5, 1, 
                                    1, 0.5, 1, 0.5, 1};
    ...
    verse_pnd.apply_scale(major);
    
    //PMG
    PMG verse_gen(4, 120, verse, verse_pnd);
    std::vector<Note> verse_melody = verse_gen.run(0);
    //print chord
    // std::cout << probs[0] << "\n";
    ...
    outputfile.write("prob_gen.mid"); // write Standard MIDI File twinkle.mid
    return 0;
}
