...
int main(int argc, char** argv)
{
   ...
   outputfile.setTicksPerQuarterNote(TPQ);
    ...
    Scale verse_scale(60, MINOR);
    Section verse(verse_scale, verse_rhythm, verse_prog);

    // add_progression(&outputfile, 1, verse_prog, 0);

    //generate melody 
    Arpeggiator verse_generator;
    
    // std::vector<Note> verse_melody = verse_generator.run();
    // // add_consecutive_notes(&outputfile, 1, verse_melody, 0);
    
    // //print chord
    // std::cout << verse_prog.get_chord_at(12.5).get_chord()[0].get_tone() << "\n";
